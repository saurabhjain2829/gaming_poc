...
game_prompt = PromptTemplate(
    ...
    partial_variables={"format_instructions": GameDesignSchema.model_json_schema()},
...
def safe_run_chain(chain, inputs, retries=2):
    ...
    for attempt in range(1, retries + 1):
        try:
            retry_inputs = inputs.copy() if isinstance(inputs, dict) else {"input": inputs}
            if last_error:
                retry_inputs["previous_error"] = str(last_error)
            return chain.run(retry_inputs)
